...
WHERE
  (@DatabaseName IN ('', CHAR(60) + 'Database Name, VARCHAR(MAX), ' + CHAR(62))
        OR DB_NAME([request].[database_id]) LIKE '%' + @DatabaseName + '%')
...
, [process].[hostname]      AS [Host Name]
, [process].[loginame]      AS [Login Name]
FROM
  sys.sysprocesses AS [process]
WHERE
  (@DatabaseName IN ('', CHAR(60) + 'Database Name, VARCHAR(MAX), ' + CHAR(62))
        OR DB_NAME([dbid]) LIKE '%' + @DatabaseName + '%')
ORDER BY
  [process].[waittime] DESC
